...
export class DatabaseServer extends pulumi.ComponentResource implements IDatabaseServer {
  ...
  createUser(username: string) {
    ...
    return pulumi.output(pulumi.all([ user, password ])
      .apply(([ user, password ]) => ({
        username: user.name,
        password: password.result,
      })));
  ...
  createDatabase(dbname: string) {
    ...
    return pulumi.output(pulumi.all([ database, this.instance ])
